...
c      write(*,*) 'FABIO -- NetCDF: creating parcel_id dimension'
      nc_status = nf_def_dim(ncid,'parcel_id', NPARCEL, prcl_dimid)
      if (nc_status .ne. nf_noerr) call handle_err(nc_status)

c      write(*,*) 'FABIO -- NetCDF: creating time dimension'
...
c      write(*,*) "FABIO -- NetCDF: compressing variables"      
...
c      write(*,*) "FABIO -- starting main computational loop"
...
c          write(*,*) "FABIO -- Invoking fcstcur_1hr"
...
c----------------------------------------------------------------------
        ...
        do n=1,nmax
           if(idepth.eq.0) then
c              write(*,*) "FABIO -- Using water currents for depth 0"
              uadv(m,n)=usrf(m,n)
              vadv(m,n)=vsrf(m,n)
           else if(idepth.eq.1) then
c              write(*,*) "FABIO -- Using water currents for depth 1"
              uadv(m,n)=u10(m,n)
              vadv(m,n)=v10(m,n)
           else if(idepth.eq.2) then
c              write(*,*) "FABIO -- Using water currents for depth 2"
              uadv(m,n)=u30(m,n)
              vadv(m,n)=v30(m,n)
           else if(idepth.eq.3) then
c              write(*,*) "FABIO -- Using water currents for depth 3"
              uadv(m,n)=u120(m,n)
              vadv(m,n)=v120(m,n)
           end if   
        end do
...
c----------------------------------------------------------------------
c            write(*,*) "FABIO -- Compute spreading displacement"
            xds=0.d0
            yds=0.d0
            if (is(i).eq.1) then
              if(i.le.npcl0) then
...
c----------------------------------------------------------------------
c            write(*,*) "FABIO -- Compute diffusion displacement"
            m=int(px(i)+0.5d0)
            n=int(py(i)+0.5d0)
...
c            write(*,*) "FABIO -- compute adv. displ. of surf"
c            write(*,*) "         and dispersed parcels"
            
	    if(is(i).ne.3) then

c               write(*,*) "FABIO -- Invoking intrpl several times"
...
c        write(*,*) "FABIO -- NetCDF: Adding values to NC file"
...
c     &                                        u30,v30,u120,v120)
c        write(*,*) 'FABIO -- Invoking readfcst_1hr'
...
c        write(*,*) 'FABIO -- readfcst_1hr started!'
...
c        write(*,*) 'FABIO -- initializing sst, us, vs, etc to 0'
c        write(*,*) 'FABIO -- dimension ', mm, ' x ', nm
	do m=1,mm
	do n=1,nm
	  sst(m,n)=0.d0
c	  wx(m,n)=0.d0
...
c        write(*,*) 'FABIO -- reading sst, us, vs etc'
      do i=1,ndata
	  read(71,*) alat,alon,st,u,v,u1,v1,u3,v3,u4,v4
	  m=int((alon-alon1)/dlon+1.1d0)
	  n=int((alat-alat1)/dlat+1.1d0)
...
c	  wy(m,n)=wy1
	  ...
	  v120(m,n)=v4
c          write(*,*) 'FABIO -- ======================'
c          write(*,*) 'FABIO -- m: ', m, ' n: ', n
c          write(*,*) 'FABIO -- alat: ', alat, ' alon: ', alon
c          write(*,*) 'FABIO -- u: ', u, ' v: ', v
c          write(*,*) 'FABIO -- u1: ', u1, ' v1: ', v1
c          write(*,*) 'FABIO -- u3: ', u3, ' v3: ', v3
c          write(*,*) 'FABIO -- u4: ', u4, ' v4: ', v4
...
c
c	 now interpolate data onto medslik grid
c        write(*,*) 'FABIO -- interpolating sst on medslik grid'
	call interpol(sst,alon1,alat1,dlon,dlat)
c	call interpol(wx,alon1,alat1,dlon,dlat)
c     call interpol(wy,alon1,alat1,dlon,dlat)

c        write(*,*) 'FABIO -- interpolating us on medslik grid'
	call interpol(us,alon1,alat1,dlon,dlat)

c        write(*,*) 'FABIO -- interpolating vs on medslik grid'
	call interpol(vs,alon1,alat1,dlon,dlat)

c        write(*,*) 'FABIO -- interpolating u10 on medslik grid'
	call interpol(u10,alon1,alat1,dlon,dlat)

c        write(*,*) 'FABIO -- interpolating v10 on medslik grid'
	call interpol(v10,alon1,alat1,dlon,dlat)

c        write(*,*) 'FABIO -- interpolating u30 on medslik grid'
	call interpol(u30,alon1,alat1,dlon,dlat)

c        write(*,*) 'FABIO -- interpolating v30 on medslik grid'
	call interpol(v30,alon1,alat1,dlon,dlat)
        
c        write(*,*) 'FABIO -- interpolating u120 on medslik grid'
	call interpol(u120,alon1,alat1,dlon,dlat)
        
c        write(*,*) 'FABIO -- interpolating v120 on medslik grid'
	call interpol(v120,alon1,alat1,dlon,dlat)
c
